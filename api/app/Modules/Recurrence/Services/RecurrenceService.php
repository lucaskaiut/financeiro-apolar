...
class RecurrenceService
{
    public function paginate(int $perPage = 15, ?string $search = null): LengthAwarePaginator
    {
        return Recurrence::query()
            ->with(['bankAccount:id,uuid,name', 'costCenter:id,uuid,name', 'category:id,uuid,name', 'subcategory:id,uuid,name'])
            ->withCount('accounts')
    ...
    /**
     * @param  array{type: string, description: string, counterparty?: ?string, bank_account_id: string, cost_center_id?: ?string, category_id: string, subcategory_id?: ?string, value: numeric, frequency: string, start_date: string, end_date?: ?string, max_occurrences?: ?int, day_of_month?: ?int, status?: string}  $data
     */
