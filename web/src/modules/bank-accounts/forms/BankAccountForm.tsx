...
import type { BankAccountPayload } from '../services/bank-accounts.service'
...
interface BankAccountFormProps {
  ...
  submitting: boolean
  onSubmit: (payload: BankAccountPayload) => Promise<unknown>
}

export function BankAccountForm({ mode, defaultValues, submitting, onSubmit }: BankAccountFormProps) {
      ...
      <CardContent>
        <Form form={form} onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <ButtonLink to="/bank-accounts" variant="secondary">
              Cancelar
            </ButtonLink>
            <Button type="submit" loading={submitting}>
              {mode === 'create' ? 'Criar conta bancária' : 'Salvar alterações'}
            </Button>
          </div>
        </Form>
      </CardContent>
